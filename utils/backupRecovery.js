...
class BackupRecovery {
  ...
  async createFullBackup(options = {}) {
    ...
    try {
      ...
      // Download and add each file to backup
      ...
      archive.append(JSON.stringify(systemConfig, null, 2), { name: 'system-config.json' });
  ...
  async createIncrementalBackup(lastBackupTimestamp, options = {}) {
    ...
    try {
      ...
      const modifiedFiles = await this.getModifiedCloudinaryFiles(lastBackupTimestamp);
      
      if (modifiedFiles.length === 0) {
        logger.info('No files modified since last backup', { lastBackupTimestamp });
        return null;
      }
  ...
  async restoreFromBackup(backupId, options = {}) {
    ...
    try {
      ...
      const isValid = await this.verifyBackupIntegrity(backupInfo.path, backupInfo.checksum);
      if (!isValid) {
      ...
      for (const fileInfo of metadata.files) {
        try {
          ...
          failedFiles.push({ publicId: fileInfo.publicId, error: fileError.message });
        }
      }
  ...
  async updateBackupMetadata(backupInfo) {
    try {
      ...
      logger.error('Failed to update backup metadata', { error: error.message });
    }
  }
  ...
  async cleanupDirectory(dirPath) {
    try {
      ...
      logger.warn('Failed to cleanup directory', { dirPath, error: error.message });
    }
  }
  ...
  scheduleAutomaticBackups(options = {}) {
    const fullBackupInterval = options.fullBackupInterval || 7 * 24 * 60 * 60 * 1000; // 7 days
    const incrementalInterval = options.incrementalInterval || 24 * 60 * 60 * 1000; // 1 day
    ...
    setInterval(async () => {
      try {
        ...
        if (lastBackup) {
          ...
          const result = await this.createIncrementalBackup(lastBackup.timestamp);
          if (result) {
    ...
    try {
      ...
      for (const backup of oldBackups) {
        try {
          ...
          logger.info('Deleted old backup', { backupId: backup.backupId, name: backup.name });
        } catch (error) {
      ...
      await fs.writeFile(this.metadataFile, JSON.stringify(remainingBackups, null, 2));
