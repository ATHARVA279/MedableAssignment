...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const totalStorageUsed = summary.reduce(
      (sum, user) => sum + user.storageUsed,
      0
    );
    const totalFilesUsed = summary.reduce(
      (sum, user) => sum + user.filesUsed,
      0
    );
    const usersOverLimit = summary.filter(
      (user) => user.storagePercentage >= 100
    ).length;
    const usersNearLimit = summary.filter(
      (user) => user.storagePercentage >= 75 && user.storagePercentage < 100
    ).length;
    ...
    res.json({
      ...
      statistics: {
        ...
        usersNearLimit,
        averageStorageUsage:
          totalUsers > 0 ? Math.round(totalStorageUsed / totalUsers) : 0,
      },
      pagination: {
        limit: limitNum,
        total: totalUsers,
