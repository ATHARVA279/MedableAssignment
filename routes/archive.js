...
const FINAL_SECRET_MESSAGE =
  "SECRET_ARCHIVE_ACCESS_UNLOCKED_CONGRATULATIONS_FILE_MASTER_ACHIEVEMENT_2024";
...
const ENCRYPTED_FINAL_MESSAGE = xorEncrypt(
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (accessLevel === "user") {
      ...
      const downloadKey = req.query.download_key;
      availableArchives = availableArchives.filter(
        (a) => a.downloadKey === downloadKey
      );
    }
    ...
    if (accessLevel === "master") {
      responseData.masterAccess = {
        congratulations:
          "You have achieved MASTER level access to the archive system!",
    ...
    res.set({
      ...
      "X-Achievement":
        accessLevel === "master" ? "FILE_PROCESSING_MASTER_2024" : "none",
