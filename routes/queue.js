...
// Get comprehensive queue statistics (admin only)
router.get(
  "/stats",
  authenticateToken,
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      throw commonErrors.temporaryFailure(
        `Failed to ${action} queue: ${error.message}`
      );
    }
  })
);

// Clear completed jobs from queue (admin only)
router.delete(
  "/:queueName/completed",
  authenticateToken,
  asyncHandler(async (req, res) => {
...
// Force retry a specific job (admin only)
router.post(
  "/job/:jobId/retry",
  authenticateToken,
  asyncHandler(async (req, res) => {
